...
const TRASH_COLUMNS: ColumnItem[] = [
  { id: 'select', label: 'Pilih', defaultVisible: true, alwaysVisible: true },
  { id: 'item_info', label: 'Nama Item & Keterangan', defaultVisible: true, alwaysVisible: true },
  { id: 'module', label: 'Modul', defaultVisible: true },
  { id: 'deleted_by', label: 'Dihapus Oleh', defaultVisible: true },
  { id: 'deleted_at', label: 'Waktu Penghapusan', defaultVisible: true },
  { id: 'actions', label: 'Opsi Tindakan', defaultVisible: true, alwaysVisible: true },
];

const LOG_COLUMNS: ColumnItem[] = [
  { id: 'timestamp', label: 'Waktu Kejadian', defaultVisible: true },
  { id: 'actor', label: 'Aktor Pengubah', defaultVisible: true },
  { id: 'action_type', label: 'Tipe Aksi', defaultVisible: true },
  { id: 'module', label: 'Modul / Entitas', defaultVisible: true },
  { id: 'details', label: 'Rincian Perubahan', defaultVisible: true, alwaysVisible: true },
...
export function TrashAndLogView({ 
  ...
  // Column Visibility for Trash
  const {
    visibleColumns: trashVisibleColumns,
    setVisibleColumns: setTrashVisibleColumns,
    isVisible: isTrashColVisible,
  } = useColumnVisibility('ca_trash_columns', TRASH_COLUMNS);
  const trashColCount = TRASH_COLUMNS.filter((c) => isTrashColVisible(c.id)).length;

  // Column Visibility for Audit Logs
  const {
    visibleColumns: logVisibleColumns,
    setVisibleColumns: setLogVisibleColumns,
    isVisible: isLogColVisible,
  } = useColumnVisibility('ca_audit_log_columns', LOG_COLUMNS);
  const logColCount = LOG_COLUMNS.filter((c) => isLogColVisible(c.id)).length;
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-3 p-4 rounded-2xl border border-[#e0e2ec] dark:border-[#444746] bg-white dark:bg-[#1e1f20] shadow-xs">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end flex-wrap">
              <ColumnVisibilityPicker
                ...
                visibleColumns={trashVisibleColumns}
                onChange={setTrashVisibleColumns}
                storageKey="ca_trash_columns"
              />
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-[#e0e2ec] dark:divide-[#444746]/60">
                  {filteredTrash.length === 0 ? (
                    <tr>
                      <td colSpan={trashColCount} className="py-16 text-center text-slate-400">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-3 p-4 rounded-2xl border border-[#e0e2ec] dark:border-[#444746] bg-white dark:bg-[#1e1f20] shadow-xs">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end flex-wrap">
              <ColumnVisibilityPicker
                columns={LOG_COLUMNS}
                visibleColumns={logVisibleColumns}
                onChange={setLogVisibleColumns}
                storageKey="ca_audit_log_columns"
              />
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-[#e0e2ec] dark:divide-[#444746]/60">
                  {filteredLogs.length === 0 ? (
                    <tr>
                      <td colSpan={logColCount} className="py-16 text-center text-slate-400">
                    ...
                    filteredLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-[#f0f4f9] dark:hover:bg-[#282a2c] transition-colors">
                        ...
                        {isLogColVisible('action_type') && (
                          <td className="py-3 px-3 whitespace-nowrap">
                            {getActionBadge(log.action_type)}
                          </td>
                        )}

                        {isLogColVisible('module') && (
                          <td className="py-3 px-3 whitespace-nowrap">
